...
class CArreraGazelle:
    ...
    def addSoft(self, mode: int, name: str):
        ...
        5 : note
        """
        if ((self.__linuxOS == False) and (self.__windowsOS == True) and (
                ...
                match mode:
                    case 1:  # Normal
                        if (name != ""):
                            ...
                            if (sortie == True):
                                self.__fileJsonUser.EcritureJSONDictionnaire("dictSoftWindows", name,
                                                                             self.__softWin.getName())
                                return True
                            else:
                                return False
                        else:
    ...
    def supprSoft(self, mode: int, name: str):
        ...
        # Creation listFlag
        if ((self.__linuxOS == False) and (self.__windowsOS == True)):
            listFlag = ["dictSoftWindows", "wordWindows", "exelWindows", "diapoWindows", "browserWindows",
                        "noteWindows", "musicWindows"]
        else:
            if ((self.__linuxOS == True) and (self.__windowsOS == False)):
                listFlag = ["dictSoftLinux", "wordLinux", "exelLinux", "diapoLinux", "browserLinux", "noteLinux",
                            "musicLinux"]
            else:
                return False
        match mode:
