# Documentation de SIX

## Fonctions spécifiques à SIX

### Fonction Mute 

Arrera Six est un assistant vocal qui base toute sa communication avec l'utilisateur sur la voix. Si le système de trigger word est activé, l'assistant écoute toujours les entrées micro.

Pour que l'utilisateur puisse désactiver cela, il lui suffit de dire "mute" à Six. Il se met alors en pause et ne sera réveillé que par une action de l'utilisateur sur l'interface de l'assistant.

### Fonction Trigger Word

Le Trigger Word (ou mot de réveil) est la fonctionnalité clé permettant une utilisation mains-libres de l'assistant. Lorsqu'elle est activée, Six analyse le flux audio du microphone en continu afin de détecter un mot qui a été rentré dans les paramètres.
